...
namespace Renderer {
    ...
    {
        s_pDefaultFont = Font::LoadFont (strFontPath, (float)nSizeY);
    ...
    void DrawGeneric (const RendererVertex* vertexBuffer, unsigned int nVertexCount, unsigned int* indexBuffer, unsigned int nIndexCount, unsigned int nTexId)
    {
        ...
        for (unsigned int i = 0; i < nVertexCount; i++)
        {
            ...
            vertexOffset[i].SetTexId ((float)nTextureSlot);  
        }
    ...
    /////       Left Align
    ////////////////////////////////////////
    static void DrawTextLine_LeftAligned (const char* strText, std::size_t strLen, const rmg::vec3& posOriginal, float scale, const Color& color, const FontStyle& style, Font* pFont)
    {
    ...
    static void DrawTextLine_CenterAligned (const char* strText, int strLen, const rmg::vec3& posOriginal, float scale, const Color& color, const FontStyle& style, Font* pFont)
    {
        ...
        if (strLen % 2 == 0)
        {
            ...
            if (fcMiddleLeft && fcMiddleRight)
            {
                ...
                const float leftcharPos = (float)(fcMiddleLeft->Bearing.x + fcMiddleLeft->Size.x);
                //Relative x position of the left edge of letter C
                const float rightcharPos = (float)(fcMiddleLeft->Advance + fcMiddleRight->Bearing.x);
        ...
        else
        {
            ...
            if (fcMiddleLeft && fcMiddle)
            {
                ...
                const float leftcharPos = (float)(fcMiddleLeft->Bearing.x + fcMiddleLeft->Size.x);
                //Relative x position of the left edge of letter C
                const float rightcharPos = (float)(fcMiddleLeft->Advance + fcMiddle->Bearing.x);
                float fPadding = (rightcharPos - leftcharPos) * scale;
                fPadding /= 2.0f;
                posLeft.x -= fPadding;
                posRight.x += fPadding; 
